...
//    }
    ...
    public void writeItem(ElasticContext ec, MetadataDocument mdoc,
        ...
        if (mdoc.getRequiresSourceWrite() && mdoc.hasSuppliedJson()) {
          // TODO   possibly Store HTML from where JSON LD is extracted
          // Use mdoc.getRequiresXmlWrite() as proxy for mdoc.getRequiresSourceDocWrite()
          // if (mdoc.getRequiresXmlWrite() && mdoc.hasSuppliedJson()) {
            byte[] bytes = null;
            if (mdoc.hasSuppliedJson()) {
                bytes = mdoc.getSuppliedJson().getBytes("UTF-8");
            }
            JsonObjectBuilder meta = prepareXmlMeta("application/ld+json", bytes);
            if (ec.getUseSeparateXmlItem()) {
                JsonObjectBuilder xmlItem = Json.createObjectBuilder();
                xmlItem.add(FieldNames.FIELD_SYS_CLOB, mdoc.getSuppliedJson());
                xmlItem.add(FieldNames.FIELD_SYS_ITEMID, itemId);
                xmlItem.add(FieldNames.FIELD_SYS_META, meta.build().toString());
                // FieldNames.FIELD_SYS_ITEMID, getItemId(),
                String xmlUrl = client.getXmlUrl(indexName, ec.getXmlIndexType(), itemId);
                ...
                JsonObjectBuilder item = JsonUtil.newObjectBuilder(itemJson);
                item.add(FieldNames.FIELD_SYS_SOURCE, mdoc.getSuppliedJson());
                item.add(FieldNames.FIELD_SYS_SOURCEMETA, meta);
                itemJson = item.build().toString();
            }
        }

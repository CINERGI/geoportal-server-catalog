...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="gmd:MD_Metadata | gmi:MI_Metadata" mode="test">
        ...
        <div class="container-fluid page-header">
            <!--<div class="row">-->
                <!--&lt;!&ndash;<a href="http://www.iedadata.org"><img onclick="http://www.iedadata.org" src="http://app.iedadata.org/images/ieda_maplogo.png" alt="IEDA"/></a>&ndash;&gt;-->
                <!--&lt;!&ndash; partner system logo for metadata contact if present-->
			<!--not populated for MGDL or USAP, so comment out for now &ndash;&gt;-->
                <!--&lt;!&ndash;<xsl:if test="//gmd:contact//parent::*[gmd:CI_OnLineFunctionCode='logo graphic'][1]/preceding-sibling::gmd:linkage/gmd:URL">-->
				...
				<!--</xsl:element>-->
			<!--</xsl:if> -\->-->
			<!--<!-\-<xsl:value-of select="$isopath"/>-\->&ndash;&gt;-->
            <!--</div>-->
...
-->
        ...
        <div class="container-fluid">
            ...
            that describe the metadata. -->
                ...
                <div class="col-sm-6" data-spy="scroll" data-target="#metadataPanel" id="panel">
...
-->
                ...
                <!--displa map if have geospatial extent id="right-side"-->
        ...
        <dd>
            <xsl:choose>
                ...
                <xsl:when
                    ...
                    <xsl:choose>
                        <xsl:when
                                test="string-length(gmd:type/gmd:MD_KeywordTypeCode[@codeListValue = '005' or @codeListValue = 'theme']/text())>0">
                            <dt>
                                <span class="element">Theme keywords (<xsl:value-of
                    ...
                    <xsl:if test="gmd:keyword">
                        <dt>
                            <xsl:for-each select="gmd:keyword">
                                <xsl:if test="position() = 1">
                                    <span class="element">keyword term:</span>&#x2003;
                                </xsl:if>
                                <xsl:call-template name="CharacterString"/>
                                <xsl:if test="not(position() = last())">,</xsl:if>
                            </xsl:for-each>
            ...
            <dd>
                <dl>
                    <xsl:if test="srv:name/gco:MemberName">
                        <dt>
                            <span class="element">Parameter name:</span>&#x2003;
                        </dt>
...
<xsl:template match="TypeName" mode="iso19139">
        ...
        <dl>
            <dd>
                ...
                <xsl:for-each select="gmd:processingLevelCode/gmd:MD_Identifier">
                    <dt>
                        <span class="element">processing Level Code</span>&#x2002;
                    </dt>
                    <xsl:apply-templates select="." mode="iso19139"/>
                </xsl:for-each>
    ...
    <xsl:template match="gmd:DQ_ConformanceResult" mode="iso19139">
        <dd>
            <dt>
                <span class="element">Conformance Result</span>&#x2002;
            </dt>
            <dd>
                <dl>
    ...
    <xsl:template match="gmd:MD_Distribution" mode="iso19139">
        ...
        <xsl:if test="count(../../gmd:distributionInfo) &gt; 1">
            <dt>
                <h2>Multiple distribution methods:
                    <xsl:value-of select="position()"/>
                </h2>
            </dt>
        </xsl:if>

...
// check
    evalProps: function (task, obj, contextNode, name, jsonpathExpression, options) {
        if (!contextNode) return;
        var self = this;
        var node = this.getNode(task, contextNode, jsonpathExpression);
        // it's JSON... sometimes it is a string
        if (node.constructor === String || typeof node === "string"){
            self.writeMultiProp(obj, name, node, options);
            return;
        }
    ...
    forEachNode: function (task, contextNode, jsonpathExpression, callback) {
        ...
        print (typeof nl,Array.isArray(nl), nl);
        try {
            ...
            print ("object not array, or string ");
        }
    },
...
// check
    getNodeText: function (node) {
        var v;
        print("getNodeText Start");
        print(node);
        if (typeof node !== "undefined" && node !== null) {
            print(typeof node);
            print(node.constructor);
            print(node.constructor == String);
            print(node.constructor == Array);
            print(node.constructor == Object);
            if (node.constructor == String || typeof node == "string") {

                v = this.Val.trim(node);
                print("getNodeText string: " + node);
            } else {
                if (node.constructor == Array || Array.isArray(node)) {
                    print("getNodeText array:" + node.toString());
                    print("array");
                    if (node.length > 0) {
                        v = "";
                ...
                } else if (node.constructor == Object || typeof node == "object") {
                    print("getNodeText object:" + typeof node);
                    print(typeof node);
                    print(node.values);
                    v = "";
                    if (node.name){
                        v = this.getNodeText(node.name);
                    } else {
                    ...
                    print("getText for Node.. not a string")
                }



        }
        print("getNodeText return type:" + typeof v);
        if (typeof v === "string") {
            return v;
        }
    }
